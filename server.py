...
_CATEGORY_RULES = [
    (
        "court_tennis",
        "코트/테니스",
        ("테니스화", "하드 코트", "코트 ff", "레졸루션", "챌린지", "베이퍼 12", "court ff", "gel resolution"),
    ),
    (
        "trail_outdoor",
        "트레일/아웃도어",
        (
            "트레일", "trail", "하이킹", "hiking", "아웃도어", "outdoor",
            "클라우드록", "cloudrock", "cloudhorizon", "cloudvista", "cloudultra",
            "acg", "xa 프로", "xt-6", "그래블", "gravel", "norda", "bucket mountain",
        ),
    ),
    (
        "road_running",
        "러닝",
        (
            "러닝화", "running", "road racing", "마라톤", "marathon", "클라우드러너",
            "클라우드몬스터", "클라우드서퍼", "클라우드스위프트", "클라우드플로우",
            "cloudboom", "mach", "님버스", "spike", "스파이크",
        ),
    ),
    (
        "training_fitness",
        "트레이닝/피트니스",
        ("트레이닝", "training", "피트니스", "fitness", "프리게임", "마인드", "cloud x", "클라우드 x", "클라우드펄스"),
    ),
    (
        "lifestyle_fashion",
        "라이프스타일/패션",
        (
            "라이프스타일", "올데이", "스니커즈", "슈즈", "신발", "로저", "코르테즈",
            "젯", "rx", "메리제인", "brick", "jogger", "클라우드노바", "cloudnova",
            "클라우드솔로", "cloudsolo", "클라우드틸트", "cloudtilt", "cloud 6",
            "cloudflow 5 ad", "느바", "어드밴스드",
        ),
    ),
]


def _shoe_category(p: dict) -> dict:
    """제품명/상세명/URL 기반 5개 분석 카테고리 자동 분류."""
    haystack = " ".join(
        str(p.get(key) or "")
        for key in ("brand", "product_name", "display_name", "source_url", "final_url", "sku", "product_id")
    ).lower()
    for key, label, keywords in _CATEGORY_RULES:
        if any(token.lower() in haystack for token in keywords):
            matched = [token for token in keywords if token.lower() in haystack]
            return {
                "category_group": key,
                "category_label": label,
                "category_tags": matched[:3],
            }
    return {
        "category_group": "lifestyle_fashion",
        "category_label": "라이프스타일/패션",
